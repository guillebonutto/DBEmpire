...
async function run() {
    ...
    for (let month = 0; month < 6; month++) {
        ...
        for (const e of expenses) {
            ...
            if (eMs < startMs) {
                if (isYield) {
                    prevYields += val; // negative
                } else if (!isStock) {
                    prevExpCaja += val;
                }
            } else if (eMs <= endMs) {
                if (isYield) {
                    bankYields += val; // negative
                } else {
        ...
        // histBalCaja should add yields as positive income. Since prevYields is negative, we subtract it:
        const histBalCaja = prevIncome - prevExpCaja - prevYields;
        
        // netCaja should add current yields as positive income. Since bankYields is negative, we subtract it:
        const netCaja = histBalCaja + totalSales - bankYields - totalExpensesCaja;

        console.log(`Month: 2026-${month+1} | Net Caja: $${netCaja.toFixed(2)}`);
    }
}
run();
