...
old_logic = """    const generateAiInsights = async (customQuery = null) => {
        setIsAiLoading(true);
...
new_logic = """    const generateAiInsights = async (customQuery = null) => {
        setIsAiLoading(true);
        if (!customQuery) setAiAdvice(null);
        try {
            // 1. Fetch Top Products sold this week to give AI context
...
if old_logic in code:
    ...
    print("Could not find prompt code using replace")
...
if old_ui in code:
    ...
    print("Could not find UI using exact replace")

old_styles = "closeModalBtnText: { color: '#aaa', fontSize: 12, fontWeight: '900', letterSpacing: 1 }"
new_styles = """closeModalBtnText: { color: '#aaa', fontSize: 12, fontWeight: '900', letterSpacing: 1 },
...
print("Replacement complete. Double checks done.")
